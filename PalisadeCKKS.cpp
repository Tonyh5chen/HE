...
int main()
{
	...
	for(int i = 0; i < N; i++)
	{
		complex<double> a = (rand()/(double(RAND_MAX))*10); //palisade found overflow if the integer is too large
		acc.push_back(a);

		complex<double> b = (rand()/(double(RAND_MAX))*10);
		initial_velocity.push_back(b);

		complex<double> c = (rand()/(double(RAND_MAX))*10);
		times.push_back(c);
	}
