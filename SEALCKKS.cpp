...
void example_ckks_basics()
{
    ...
    for(int i = 0; i < N; i++)
    {
        ...
        double b = (rand()/(double(RAND_MAX))*10);
        initial_velocity.push_back(b);

        double c = (rand()/(double(RAND_MAX))*10);
        times.push_back(c);
    }
    ...
    print_vector(acc, 3, 4);

    cout << "Initial Velocity: " << endl;
    print_vector(initial_velocity, 3, 4);

    cout << "Time: " << endl;
    print_vector(times, 3, 4);

    cout << " Final Velocity: " << endl;
    print_vector(final_vel, 3, 4);
